...
def fetch_outgoing(wallet, ts_start, ts_end, progress_bar, status_text):
    ...
    while True:
        ...
        progress_bar.progress(min(fetched / max(total, 1), 1.0) * 0.4)
...
with st.sidebar:
    ...
    wallet  = st.text_input("Wallet Address", placeholder="Enter TRON address (T...)")
        ...
        try:
            ...
            if not txs:
                ...
                with ch1:
                    ...
                    st.bar_chart(df.set_index("datetime")[["trx_burned"]].sort_index())

                with ch2:
                    st.caption("Energy Consumed per Transaction")
                    st.bar_chart(df.set_index("datetime")[["energy_total"]].sort_index())
                ...
                st.download_button(
                    "⬇️ Download CSV", csv,
                    file_name=f"usdt_outgoing_{wallet[:10]}.csv",
                    mime="text/csv"
                )
